 ...
 * Sync Version
 *
 * Synchronizes the version number from root package.json to:
 * - electron-app/package.json
 * - electron-builder.json (product name)
 *
...
// Update electron-builder.json product name
...
if (fs.existsSync(electronBuilderPath)) {
  ...
  const productName = 'Client Data Management';

  if (electronBuilder.productName !== productName) {
    console.log(`Updating productName to "${productName}"`);
  ...
  console.log('Warning: electron-builder.json not found');
}
